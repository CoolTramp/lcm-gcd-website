...
export class Canonical extends PreparingCalculations {
  ...
  _addDegreesInCanonical(numberIndex, primeNumber, degree) {
    this._canonical[this._numbers[numberIndex]][primeNumber] = {
      degree: degree,
    };
  }
  ...
  _updateData(primeNumber, degree) {
    this._data[primeNumber].minDegree = Math.min(
      degree,
      this._data[primeNumber]?.minDegree
    );
    this._data[primeNumber].maxDegree = Math.max(
      degree,
      this._data[primeNumber]?.maxDegree
    );
