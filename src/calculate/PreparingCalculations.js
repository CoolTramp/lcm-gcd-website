...
export class PreparingCalculations {
  ...
  constructor(...numbers) {
    ...
    this._primeFactorization = new PrimeFactorization(
      this._trackingDivideHistory
    );
  ...
  _addDividingHistoryStringToFormattedStringsArray() {
    let tuples = this._trackingDivideHistory.getHistoryOfDividingNumber();

    let paddedString = this._padFormatter.creatStringFrom(tuples);
  ...
  _checkInput(numbers) {
    if (numbers.length <= 1) {
      throw new Error(
        "there must be at least 2 different digits for calculating LCD"
      );
    }
